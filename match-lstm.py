...
w = tf.get_variable("w", shape = [l, 1], initializer=tf.random_normal_initializer())
...
v = tf.get_variable("v", shape = [l, 1], initializer=tf.random_normal_initializer())
...
for i in range(epoch):
    for (paraMat , ques) in create_embeddings.main():
        paraMat = np.reshape(paraMat, (1,396, 50))
        ques = np.reshape(ques, (1,33,50))
